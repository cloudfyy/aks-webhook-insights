...
func main() {
	...
	subject := pkix.Name{
		...
		Organization:       []string{"ms"},
		OrganizationalUnit: []string{"ms"},
	}
	ca := &x509.Certificate{
	...
	dnsNames := []string{"aks-webhook",
		"aks-webhook.default",
		"aks-webhook.default.svc",
		"aks-webhook.default.svc.cluster.local",
	}
	commonName := "aks-webhook.default.svc"
	// 服务端的证书配置
	...
	if err := util.WriteFile("/etc/webhook/certs/tls.crt", serverCertPEM.Bytes()); err != nil {
		log.Panic(err)
	}

	if err := util.WriteFile("/etc/webhook/certs/tls.key", serverPrivKeyPEM.Bytes()); err != nil {
		log.Panic(err)
	}
...
func CreateAdmissionConfig(caCert *bytes.Buffer) error {
	clientset, err := util.InitKubernetesCli()
	if err != nil {
		return err
	}
