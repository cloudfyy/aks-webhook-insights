...
openssl req -new -key ${tmpdir}/server-key.pem -subj '/CN=system:nodes:${title}.${namespace}.svc" /OU="system:nodes" /O=system:nodes' -out ${tmpdir}/server.csr -config ${tmpdir}/csr.conf
